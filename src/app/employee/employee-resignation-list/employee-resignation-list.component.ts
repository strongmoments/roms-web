...
@Component({
  ...
  templateUrl: './employee-resignation-list.component.html',
  styleUrls: ['./employee-resignation-list.component.scss'],
  animations: [
...
export class EmployeeResignationListComponent implements OnInit {
  globals: Globals;
  // leaveHours: number = 0;
  // form: FormGroup;
  ...
  refresh(options: ViewOptions) {
    this.employeeService
      .getAllResignationRequest(options)
      .pipe(first())
      .subscribe((result: any) => {
        this.totalRecords = result.totalElement;
  ...
  approveReject(type: number, index: number, id: any) {
    ...
    if (type == 0) {
      this.employeeService
        .acceptResignation({ id: id, comment: this.comments[index] ? this.comments[index] : '' })
        .subscribe(
          (res) => {
            this.alertService.openSnackBar(CustomMessage.resignationAccepted, false);
            this.refresh(this.getDefaultOptions());
      ...
      this.employeeService
        .rejectResignation({ id: id, comment: this.comments[index] ? this.comments[index] : '' })
        .subscribe(
          (res) => {
            this.alertService.openSnackBar(CustomMessage.resignationRejected, false);
            this.refresh(this.getDefaultOptions());
            // this.router.navigate(['/dashboard']);
          },
          (error) => {
